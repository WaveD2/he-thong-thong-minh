...
def classify_vehicle_plate(plate_number: str) -> str:
    plate_number = plate_number.replace(" ", "").upper()
    
    motorcycle_pattern = r'^\d{2}[A-Z]{1,2}\d?-\d{3,4}\.\d{2}$|^\d{2}-MĐ-\d{3}\.\d{2}$'
    
    car_pattern = r'^\d{2}[A-Z]-\d{3}\.\d{2}$|^\d{2}[A-Z]-\d{3}\.\d{3}$|^[A-Z]{2}-\d{3}-\d{2}$|^[A-Z]{2}-\d{3}-\d{3}$'
    
    if re.match(motorcycle_pattern, plate_number):
        return "Xe máy"
    elif re.match(car_pattern, plate_number):
        return "Xe ô tô"
    else:
